 ...
 * Used and modified by: Ahmed Ghannam (0910337) 
 *
 ...
 * many operations. In particular, insert, peek, and decrease-key all run in
 * amortized O(1) time. dequeueMin and delete each run in amortized O(lg n)
 * time. This allows algorithms that rely heavily on decrease-key to gain
 * significant performance boosts. For example, Dijkstra's algorithm for
 ...
 * so that there is only one tree of each degree in the root list. This works by
 ...
 * populated. Whenever a conflict is discovered, the appropriate trees are
 * merged together until no more conflicts exist. The resulting trees are then
 * put into the root list. A clever analysis using the potential method can be
